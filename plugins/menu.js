...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  ...
  let arrayMenu = ['all', 'game', 'rpg', 'xp', 'fun', 'kerangajaib', 'quotes', 'jadian', 'grup', 'premium', 'anime', 'internet', 'anonymous', 'downloader', 'tools', 'sticker', 'admin', 'database', 'quran', 'audio', 'info', 'tanpakategori', 'owner']
    ...
    if (teks == '404') {
      ...
      const sections = [
      {
        ...
        rows: [
          ...
          { title: 'Sticker', rowId: `${_p + command} sticker` },
          ...
          { title: 'jadian', rowId: `${_p + command} jadian`},
...
${pe}Note: Jika ada Fitur yg Error Lapor ke owner${pe}`,
    ...
    await conn.send3ButtonImg(m.chat, await kontol, text.trim(), wm, `🏅Owner`, `${_p}owner`, `🎖info bot`, `${_p}ping`, `🎗  Donasi  🎗`, `${_p}donasi`, m)
  } catch (e) {
